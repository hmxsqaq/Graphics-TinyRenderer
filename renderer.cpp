...
void set_model_mat(const double angle, const double scale, const Vec3 &translate) {
    const auto radian = angle * M_PI / 180.0;
    const Mat<4, 4> rotation_mat({{{
...
void set_projection_mat(const double fov, const double aspect_ratio, const double zNear, const double zFar) {
    const Mat<4, 4> p2o({{{
        {zNear, 0, 0, 0},
        {0, zNear, 0, 0},
    ...
    const double angle = fov / 180.0 * M_PI;
    const double t = tan(angle / 2) * zNear;
    const double b = -t;
...
void set_viewport_mat(const int x, const int y, const int w, const int h) {
    ViewportMatrix = {{{
        {w / 2.0, 0, 0, x + w / 2.0},
        {0, h / 2.0, 0, y + h / 2.0},
        {0, 0, 0.5, 0.5},
...
Renderer::Renderer(const int width, const int height, const int bbp)
...
void Renderer::set_pixel(const int x, const int y, const Color &color) {
    if (frame_buffer_.empty() || x < 0 || y < 0 || x >= width_ || y >= height_) {
...
void Renderer::draw_line(Vec2 p0, Vec2 p1, const Color &color) {
    ...
    const int x0 = std::floor(p0.x), y0 = std::floor(p0.y);
    const int x1 = std::floor(p1.x), y1 = std::floor(p1.y);
    const int dx = x1 - x0;
    const int dy = y1 - y0;
...
void Renderer::draw_triangle_linesweeping(Vec2 p0, Vec2 p1, Vec2 p2, const Color &color) {
    ...
    const double total_height = p2.y - p0.y;
    for (int y = 0; y <= total_height; ++y) {
        const bool second_half = y > p1.y - p0.y || p0.y == p1.y;
        const double segment_height = second_half ? p2.y - p1.y : p1.y - p0.y;
...
#pragma omp parallel for // omp optimization (I hope so)
    for (int x = bbox_min[0]; x <= bbox_max[0]; x++) {
        for (int y = bbox_min[1]; y <= bbox_max[1]; y++) {
            const Vec3 bc_screen = get_barycentric2D(t_vert_screen_vec2, {static_cast<double>(x), static_cast<double>(y)});
...
Vec3 Renderer::get_barycentric2D(const std::array<Vec2, 3> &t_vert, const Vec2 &p) {
    // calculate by area
    const double x0 = t_vert[0][0], y0 = t_vert[0][1];
    const double x1 = t_vert[1][0], y1 = t_vert[1][1];
    const double x2 = t_vert[2][0], y2 = t_vert[2][1];
    const double t_area = x0 * (y1 - y2) + x1 * (y2 - y0) + x2 * (y0 - y1);
    if (std::abs(t_area) < 1e-3) return {-1, 1, 1}; // degenerate check
    const double u = (p.x * (y1 - y2) + (x2 - x1) * p.y + x1 * y2 - x2 * y1) / t_area;
    const double v = (p.x * (y2 - y0) + (x0 - x2) * p.y + x2 * y0 - x0 * y2) / t_area;
    return {u, v, 1.0 - u - v};
}

// judge by cross product (like GAMES101)
bool Renderer::is_inside_triangle_cross_product(const Vec2 *t, const Vec2 &P) {
    const Vec2 AB = t[1] - t[0], BC = t[2] - t[1], CA = t[0] - t[2];
    const Vec2 AP = P - t[0],    BP = P - t[1],    CP = P - t[2];
    const double z1 = cross(AB, AP);
    const double z2 = cross(BC, BP);
    const double z3 = cross(CA, CP);
